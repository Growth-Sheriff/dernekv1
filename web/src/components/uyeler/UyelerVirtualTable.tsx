...
export const UyelerVirtualTable: React.FC<UyelerVirtualTableProps> = ({
  ...
  const getDurumBadge = (durum: string) => {
    const durumMap: Record<string, { variant: 'default' | 'secondary' | 'error' | 'outline'; label: string }> = {
      'Aktif': { variant: 'default', label: 'Aktif' },
      'Pasif': { variant: 'secondary', label: 'Pasif' },
      'Askıda': { variant: 'outline', label: 'Askıda' },
      'İhraç': { variant: 'error', label: 'İhraç' },
    };
  ...
  const columnRenderers: Record<string, {
    ...
    kalan_borc: {
      ...
      render: (uye) => {
        ...
        return borcDurumu ? (
          <span
            className={`text-sm font-semibold ${borcDurumu.kalan_borc > 0 ? 'text-red-600' : 'text-green-600'
              }`}
          >
